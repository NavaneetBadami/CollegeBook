...
{		
   private final String UPLOAD_DIRECTORY = "C:/apache-tomcat-7.0.27/webapps/CollegeBook/img/upload"; //Path where image is going to store.  
   @Override
	...
	try
	{
       ...
       if(ServletFileUpload.isMultipartContent(req)) //process only if its multipart content.
       {
               ...
               for(FileItem item : multiparts)
               {
                   if(!item.isFormField())
                   {
                    //  String name = new File(item.getName()).getName();    //This is for to Get the default Image Name.
                   	...
                   	String imageName=userName+".jpg";                     //Concat the ".jpg" to store in .jpg Format.and write it i.e load image.
           ...
           catch (Exception ex) 
           {
             ...
             req.setAttribute("error_AP","sorry image not loaded."); //just exception when unable to store the image.
           }                          
        }       		
        else  
        {
